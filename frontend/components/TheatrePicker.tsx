...
interface TheatrePickerProps {
  movieId: string;
  theatres: Theatre[];
  mappings: MovieTheatreMapping[];
  onChoose: (theatre: Theatre) => void;
}

/** Render theatres related to the currently selected movie, without inventing availability. */
export function TheatrePicker({ movieId, theatres, mappings, onChoose }: TheatrePickerProps) {
  const availableTheatreIds = new Set(
    mappings
      .filter((mapping) => mapping.movieId === movieId)
      .map((mapping) => mapping.theatreId),
  );
  const availableTheatres = theatres.filter((theatre) => availableTheatreIds.has(theatre.id));

  return (
    <section aria-labelledby="theatre-picker-title">
      ...
      <div className="theatre-list">
        {availableTheatres.map((theatre) => (
          <button
            className="theatre-choice"
            key={theatre.id}
            onClick={() => onChoose(theatre)}
            type="button"
          >
            <strong>{theatre.name}</strong>
            <span>Available for this show</span>
          </button>
        ))}
      </div>
    </section>
